...
def evaluate_model(label_directory, output_directory):
    ...
    label_files, output_files = find_test_challenge_files(label_directory, output_directory)
...
def find_test_challenge_files(label_directory, output_directory):
    label_files = list()
    output_files = list()
...
def compute_f_measure(labels, outputs):
    ...
    if 2 * total_tp + total_fp + total_fn:
      micro_f_measure = float(2 * total_tp) / float(2 * total_tp + total_fp + total_fn)
    else:
      micro_f_measure = float('nan')

    return macro_f_measure, micro_f_measure, f_measure
